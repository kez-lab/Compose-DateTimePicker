...
`Picker<T>`에서는 `itemText`를, composite picker에서는
...
화면에 보이는 텍스트와 스크린 리더 문구가 달라야 한다면 visible text는 `itemText`로, 접근성 문구는
`accessibility.itemContentDescription`으로 분리하세요.
